...
usage() {
    ...
    echo "  up              - Start all services (PostgreSQL + Document Services)"
...
case "$1" in
    ...
    up)
        ...
        echo -e "${GREEN}PostgreSQL: postgresql://localhost:5434/document_services${NC}"
        ;;
    ...
    db-logs)
        ...
        docker logs -f document-services-postgres
        ;;
    ...
    status)
        ...
        # Test PostgreSQL
        if docker exec document-services-postgres pg_isready -U admin -d document_services > /dev/null 2>&1; then
            echo -e "${GREEN}✓ PostgreSQL (port 5434): Running${NC}"
        else
            echo -e "${RED}✗ PostgreSQL (port 5434): Not responding${NC}"
        fi
