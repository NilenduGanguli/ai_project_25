...
def page_upload_documents():
    """Page for uploading documents and generating schemas"""
    st.markdown('<h2 class="sub-header">📤 Upload Documents for Schema Generation</h2>', unsafe_allow_html=True)
    ...
    <div class="info-box">
        ...
        <ul>
            ...
            <li>The system will classify the document type automatically</li>
            <li>If no schema exists, a new schema will be generated</li>
            <li>If schema exists in review, you'll be notified</li>
            <li>If schema is active, data will be extracted immediately</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    if uploaded_files:
        ...
        if st.button("🚀 Process Documents", type="primary", use_container_width=True):
            with st.spinner("Processing documents... This may take a few minutes."):
                result = upload_document_for_schema(uploaded_files)
                
                if result:
                    status = result.get("status")
                    
                    if status == "schema_generated":
                        st.markdown("""
                        <div class="success-box">
                            <h3>✅ Schema Generated Successfully!</h3>
                            <p>A new schema has been created and is pending review.</p>
                        </div>
                        """, unsafe_allow_html=True)
                        
                        st.markdown("---")
                        
                        # Show document preview and schema side by side
                        ...
                        with col2:
                            ...
                            schema_fields = result["generated_schema"]["schema"]
                            ...
                            # Show all fields with scrollable container
                            st.markdown("**Schema Structure:**")
                            
                            # Create a scrollable container for all fields
                            for field, props in schema_fields.items():
                                field_type = props.get('type', 'unknown')
                                field_desc = props.get('description', 'No description')
                                field_required = "✓" if props.get('required', False) else "✗"
                                field_example = props.get('example', 'N/A')
                                
                                # Create individual field cards
                                st.markdown(f"""
                                <div style="margin-bottom: 10px; padding: 10px; background-color: #f8f9fa; border-left: 3px solid #0066cc; border-radius: 3px;">
                        ...
                        <div class="warning-box">
                            <h3>⏳ Schema Pending Review</h3>
                            <p>A schema for this document type is already awaiting approval.</p>
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        <div class="success-box">
                            ...
                            <p>An active schema was found and data has been extracted.</p>
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        with col2:
                            ...
                            st.json({
                                "Schema Version": result["schema_used"]["version"],
                                "Document Type": result["schema_used"]["document_type"],
                                "Country": result["schema_used"]["country"],
                                "Confidence": f"{result['classification']['confidence']:.2%}"
                            })
...
def page_extract_and_view():
    ...
    <div class="info-box">
        ...
        <ul>
            ...
            <li>System uses active schema to extract structured data</li>
    ...
    if uploaded_files and st.button("🚀 Extract Data", type="primary", use_container_width=True):
        with st.spinner("Extracting data... This may take a moment."):
            ...
            if result and result.get("status") == "extracted":
            ...
            elif result and result.get("status") == "schema_generated":
                st.info("ℹ️ No active schema found. A new schema has been generated and needs approval first.")
                # Still show the uploaded documents
                st.markdown("---")
                ...
                for idx, file in enumerate(uploaded_files):
                    with st.expander(f"Document {idx + 1}: {file.name}", expanded=idx==0):
                        file.seek(0)
                        display_document(file)
            elif result and result.get("status") == "pending_review":
                st.info("ℹ️ Schema is pending review. Please approve it first before extracting data.")
                # Still show the uploaded documents
                st.markdown("---")
                st.subheader("📄 Uploaded Documents")
                for idx, file in enumerate(uploaded_files):
...
def main():
    ...
    page = st.sidebar.radio(
        "Go to",
        ["📤 Upload Documents", "📚 View All Schemas", "✏️ Modify Schemas", "🔍 Extract & View"],
