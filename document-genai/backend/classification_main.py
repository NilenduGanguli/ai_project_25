...
Classifies PDF documents page-by-page using Google GenerativeAI
"""
import os
import base64
import uvicorn
...
app = FastAPI(
    ...
    description="Classify PDF documents into different document types using Google GenAI",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
)
...
class PDFDocumentClassifier:
    def __init__(self):
        # No need to handle API key here - LLM config will handle it
        pass
    ...
    async def classify_entire_pdf(self, pdf_data: bytes) -> ClassificationResponse:
        ...
        from src.schemas.messages import HumanMessage, SystemMessage
        ...
        # Use the same LLM configuration as the original
        llm = await get_llm(
            model_name="gemini-2.5-flash",
            model_provider="google_genai",
            temperature=0.1,
            structured_schema=ClassificationResponse
        )
        ...
        try:
            response: ClassificationResponse = await llm.ainvoke(messages)
            return response
        except Exception as e:
