...
from ..db.sync_wrapper import run_in_threadpool
...
def _calculate_next_version_sync(current_schema: DocumentSchema) -> int:
    """Synchronous version of calculate_next_version"""
...
async def calculate_next_version(current_schema: DocumentSchema) -> int:
    return _calculate_next_version_sync(current_schema)


def _find_latest_schema_version_sync(document_type: str, country: str) -> DocumentSchema:
    """Synchronous version of find_latest_schema_version"""
    session = db.session_factory()
    try:
        stmt = select(DocumentSchema).where(
            and_(
...
async def find_latest_schema_version(document_type: str, country: str) -> DocumentSchema:
    return _find_latest_schema_version_sync(document_type, country)
